...
async def test_find_all_brands(get_brand_service_impl):
    result = await get_brand_service_impl.find_all(keyword=None, page=1, size=10)
    assert len(result.items) > 0
    # change page
    result = await get_brand_service_impl.find_all(keyword=None, page=2, size=10)
    assert len(result.items) == 0
...
async def test_find_all_brands__search_by_keyword(get_brand_service_impl):
    ...
    result = await get_brand_service_impl.find_all(keyword=b1.name, page=1, size=10)
    assert len(result.items) == 2
    result = await get_brand_service_impl.find_all(keyword=b2.name, page=1, size=10)
    assert len(result.items) == 1
    result = await get_brand_service_impl.find_all(keyword="random", page=1, size=10)
    assert len(result.items) == 0
    result = await get_brand_service_impl.find_all(
        keyword="search_by_keyword_description", page=1, size=10
    )
    assert len(result.items) == 1
