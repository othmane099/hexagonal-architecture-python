...
class CategoryServiceImpl(CategoryService):
    ...
    async def create(self, dto: CreateCategoryDTO) -> CategoryResponseDTO:
        async with self.unit_of_work as uow:
            category = await uow.category_repository.find_by_name(dto.name)
            if category:
            ...
            uow.category_repository.create(category)
            await uow.commit()
            return convert_category_to_category_response_dto(category)

    async def delete(self, category_id: int) -> None:
        async with self.unit_of_work as uow:
            category = await get_existed_entity_by_id(
                uow.category_repository, category_id
            )
    ...
    async def update(self, dto: UpdateCategoryDTO) -> CategoryResponseDTO:
        async with self.unit_of_work as uow:
            category = await get_existed_entity_by_id(uow.category_repository, dto.id)
            if category.name != dto.name or category.code != dto.code:
                if category.code != dto.code:
                    tmp_category = await uow.category_repository.find_by_code(dto.code)
                    if tmp_category:
                        raise UniqueViolation("Brand code should be UNIQUE")
                    category.code = dto.code
                if category.name != dto.name:
                    tmp_category = await uow.category_repository.find_by_name(dto.name)
                    if tmp_category:
    ...
    async def find_all(
        ...
        async with self.unit_of_work as uow:
            stmt = uow.category_repository.get_find_all_stmt(
    ...
    async def delete_all_by_ids(self, dto: IdsDTO) -> DeleteAllByIdsResponseDTO:
        ids = dto.ids
        async with self.unit_of_work as uow:
            categories = await uow.category_repository.find_all_by_ids(ids)
            ...
            for bid in categories_ids:
                category = await uow.category_repository.find_by_id(bid)
                category.deleted_at = datetime.now()
            await uow.commit()
